...
uint8_t pulse_calc(uint16_t bat) {
    if(bat <= 360) return 0;
    else if(bat <= 375) return 1;
    else if(bat <= 385) return 2;
    else if(bat <= 395) return 3;
    else if(bat <= 405) return 4;
    else return 5;
}
...
int main(void) {
    DDRB |= (1 << PB0) | (1 << PB1);	//PB0 i PB1 jako wyjœcie; PB1 - LED, PB0 Vref
    PORTB |= (1 << PB1);				//w³¹cz Vref
